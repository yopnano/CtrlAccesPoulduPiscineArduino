 ...
 */
 ...
 MyNtpServer::MyNtpServer(MyITimeSource &source)
         : timeSource_(source)
     {
         // empty
     }
 
 void MyNtpServer::beginListening()
 {
     timeServerPort_.begin(NTP_PORT);
 }
 
 IPAddress MyNtpServer::getLastClientIP() {
     return lastClient_;
 }
 
 bool MyNtpServer::processOneRequest()
 {
     ...
     if (packetDataSize && packetDataSize >= NtpPacket::PACKET_SIZE)
     {
         ...
         packet.referenceId[0] = 'R';
         packet.referenceId[1] = 'T';
